...
img2mae = lambda x, y: tf.reduce_mean(tf.abs(x - y))
...
class Embedder:
    ...
    def create_embedding_fn(self):
        ...
        if self.kwargs['gaussian']:
            B = tf.random.normal([3, 256], 0.0, 10.0, seed=0)

        for freq in freq_bands:
            for p_fn in self.kwargs['periodic_fns']:
                if self.kwargs['gaussian']:
                    embed_fns.append(lambda x, p_fn=p_fn, freq=freq, B=B: p_fn(tf.matmul(x, B) * freq))
                else:
...
def get_rays(H, W, focal, c2w):
    ...
    dirs = tf.stack([(i + 0.5 - W*.5)/focal, -(j + 0.5 - H*.5)/focal, -tf.ones_like(i)], -1)
